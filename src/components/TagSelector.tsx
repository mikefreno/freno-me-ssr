  ...
  return (
    <>
      <button
        ...
        } rounded border px-4 py-2 text-white shadow-md font-light transition-all duration-300  ease-in-out active:scale-90 mt-2 md:mt-0`}
      >
        Filters
      </button>
      {showingMenu ? (
        <div
          ref={menuRef}
          className="absolute z-50 mt-12 rounded-lg bg-zinc-100 py-2 pl-2 pr-4 shadow-lg dark:bg-zinc-900"
        >
          {Array.from(props.tagMap).map(([key, value]) => (
            <div className="mx-auo my-2 flex" key={key}>
              <input
                type="checkbox"
                className=""
