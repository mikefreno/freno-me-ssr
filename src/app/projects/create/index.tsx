...
export default function ProjectCreation() {
  ...
  const postProject = async () => {
    if (titleRef.current && titleRef.current.value.length == 0) {
      ...
      if (subTitleRef.current && titleRef.current && titleRef.current.value.length > 0) {
  ...
  const saveAsDraft = async () => {
    ...
    } else if (subTitleRef.current && titleRef.current && titleRef.current.value.length > 0) {
      const res = await createProjectMutation.mutateAsync({
        title: titleRef.current.value,
        subtitle: subTitleRef.current.value.length > 0 ? subTitleRef.current.value : undefined,
        body: editorContent,
  ...
  return (
    <div className="bg-zinc-100 dark:bg-zinc-900">
      <Head>
        <title>Create Project | Freno.me</title>
          ...
          <div className="w-1/2">
            <div className="flex h-full w-full flex-col">
              ...
              <div className="-mx-24 py-8 md:-mx-36">
                <TextEditor updateContent={setEditorContent} preSet={null} />
              </div>
